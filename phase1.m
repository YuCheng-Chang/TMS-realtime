function [phase] = phase1(window, D, edge, ord, varargin)
eegsample = 1000;
%adjusted_desired_phase = 0;
%phase_tolereance = 0.1;
fs = 500;
downsample = eegsample/fs;
forwardsample = 64;
sample = 0;
phase_idx = 1;
idx = downsample;
...
allvec = nan(1,10000000);
allts = nan(1,10000000);
phase = nan(1000,forwardsample);
signal = nan(1000,window);
...
while 1
    ...
    if downsample == idx
        ...
        if mod(sample,window) == 0
            ...
            coeffs = aryule(chunk(edge+1:end-edge), ord); 
            coeffs = -coeffs(end:-1:2);
            nextvalues = zeros(1,ord+forwardsample);
            nextvalues(1:ord) = chunk(end-ord-edge+1:end-edge);
            
            for i = 1:forwardsample
                nextvalues(ord+i) = coeffs*nextvalues(i:ord+i-1)';
            end
            signal(phase_idx,:) = chunk;
            targeted_phase=find_target_phase(chunk);
            real_phase(phase_idx,:) = angle(hilbert(chunk(1:end-edge)));
            phase(phase_idx,:) = angle(hilbert(nextvalues(ord+1:end)));
            concate_phase = [real_phase,phase];
            
            plot(ax,x1,concate_phase(phase_idx,:),'r');
            hold(ax,'on');
            plot(ax,x2,signal(phase_idx,:),'b');
            grid on;
            ...
            hold(ax,'off');
            pause(0.01)
            plot(ax2,x2,signal(phase_idx,:),'b');
            op_phase_tmp=signal(phase_idx,:);
            hold(ax2,'on');
